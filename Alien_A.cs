...
namespace Space_Battleships
{
    class Alien_A
    {
        ...
        private Al_Missile Missile;

        private Random Rnd = new Random();

        PictureBox[] Pl_Bullets = new PictureBox[10];
        PictureBox[] Bullets = new PictureBox[AS];
        Boolean[] On_Move = new Boolean[AS];
        //Label LB = new Label();

        delegate void setLocationCallBack_M(int Sides);


        public Alien_A(int bounds, Point pos ,Form f, PictureBox space, PictureBox player, PictureBox[] blts)
        {
            this.Player = player;
            this.Alien_Loc = pos;
            ...
            this.Space = space;
            this.F = f;
            this.Random = Rnd.Next((Bounds / 100), (Bounds / 50));    

            if (Random <= 8)
                Random += 10;

            Alien_pic = new PictureBox();
            Alien_pic.Image = global::Space_Battleships.Properties.Resources.Alien_A;
            Alien_pic.SizeMode = PictureBoxSizeMode.Normal;
            Alien_pic.SetBounds(Alien_Loc.X, Alien_Loc.Y, 100, 120);      //100,120
            ...
            */
            ...
            Trd = new Thread(new ThreadStart(UFO_A_Move));
            Trd.Start();
        ...
        public void Gun_Visual()
        {
            if (B < AS)
            {
                ...
                Bullets[B].SetBounds(Alien_pic.Location.X + 50, Alien_pic.Location.Y + 70, 10, 15);
        ...
        private void timerTick1(object sender, EventArgs e)
        {
            ...
            {
                Kill();
                try { Missile.Explosion_Event = true; }
            ...
            if (X == 30 && Fire == true)
            {
                ...
                if (Ctr == Random)
                {
                    Point Pos = new Point(Alien_pic.Location.X + 38, Alien_pic.Location.Y + 100);
                    Missile = new Al_Missile(F, 50, Pos, Space, Player, Pl_Bullets);
                    Ctr = 0;
                }            
            }
        ...
        private void setLocation_M(int Sides)
        {
            ...
            else
            {
                ...
                if (Alien_pic.Location.X == Bounds + 150)       //950
                {
                    DirFlag = false;
                }
            }
